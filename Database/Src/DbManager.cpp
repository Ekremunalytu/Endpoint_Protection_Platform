...
QSqlDatabase DbManager::getDatabase() const {
    ...
    // Should we throw an exception or return a default-constructed QSqlDatabase?
    // Returning a default-constructed one for now, but this case might need specific error handling.
    return QSqlDatabase();
}
